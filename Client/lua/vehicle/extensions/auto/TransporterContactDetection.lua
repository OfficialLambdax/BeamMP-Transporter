...
local function getPosOffset(id)
	...
	obj:queueObjectLuaCommand(id, "if TransporterContactDetection then TransporterContactDetection.setPosOffset(" .. tostring(vehicleID) .. ", " .. serialize(data) .. ") end")
end
...
local function updateGFX(dt)
			...
			if vehiclemap[vehID] then
				...
				if vehData then
					...
					if distance < ((vehiclemap[vehID].carLength+carLength)/2)*1.01 then
						...
						--transporter.sendTransporterContact(323899)
						vehiclemap[vehID].ColState = true
					end
				end
			else
				obj:queueObjectLuaCommand(vehID,"if TransporterContactDetection then TransporterContactDetection.getPosOffset(" .. tostring(vehicleID) .. ") end")
			end
		end
	end
	...
	for ID,vehData in pairs(vehiclemap) do -- TODO: make a proper hit detection that doesn't send duplicates
		...
		if not vehData.lastColState and vehData.ColState then
			obj:queueGameEngineLua("if Transporter then Transporter.sendTransporterContact(" .. tostring(ID) .. ","..tostring(vehicleID)..") end")
			--dump("collision")
		end
